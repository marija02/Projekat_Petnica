...
img = im2double(img); % to double, Convert image to double precision.
% crop the image to the mask bounds for faster processing
stats = regionprops(mask,'BoundingBox'); %Izmerite svojstva podru?ja slike.
...
img = imquantize(img,levels); %Quantize image using specified quantization levels and output values.
% apply the mask
img(~mask) = 0;
% initialize glrlm: p(i,j)
% -  with i the amount of bin values (quantization levels)
% -  with j the maximum run length (because yet unknown, assume maximum length of image)
...
for i = 1:quantize
    ...
    % pronalazi piksele koji odgovaraju nivou kvantizacije
    BW = int8(img == i); % int8 Convert to signed 8-bit integer
    BWr = int8(img45 == i);    
    ...
    G90e = find(G90e(:));
    G135s = find(G135s(:));
    G135e = find(G135e(:)); % find - Find indices of nonzero elements.
